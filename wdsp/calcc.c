		...
		if (!InterlockedAnd(&a->restcorr_bypass, 0xffffffff))
		{
			...
			if (NULL != file)
			{
				...
				for (i = 0; i < a->util.ints; i++)
				{
					...
					for (k = 0; k < 4; k++)
					{
						if ((error == 0) && (fscanf(file, "%le", &(a->util.pm[4 * i + k])) != 1))
						{
							error = 1;
						}
					}
					for (k = 0; k < 4; k++)
					{
						if ((error == 0) && (fscanf(file, "%le", &(a->util.pc[4 * i + k])) != 1))
						{
							error = 1;
						}
					}
					for (k = 0; k < 4; k++)
					{
						if ((error == 0) && (fscanf(file, "%le", &(a->util.ps[4 * i + k])) != 1))
						{
							error = 1;
						}
					}
				...
				for (i = 0; i < a->size; i++)
				{
					...
					if ((env *= a->hw_scale) <= 1.0)
					{
						...
						else if (a->map && _InterlockedAnd (&a->ctrl.running, 1) && a->convex)
						{
							int nmin = 0;
							int nmax = a->ints;
							n = (nmin + nmax) / 2;		// missing in Thetis wdsp
							while (nmax - nmin > 1)
							{
								n = (nmin + nmax) / 2;
								if (env >= a->tmap[n])
