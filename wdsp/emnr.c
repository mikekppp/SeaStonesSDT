...
void interpM (double* res, double x, int nvals, double* xvals, double* yvals)
{
	...
	{
	int idx = 1;		// Thetis sets int idx = 0;
		double xllow, xlhigh, frac;
	while (x > xvals[idx])  idx++;		// Thetis has while (x >= xvals[idx])  idx++;
...
#define ZETA_READ_OK		0
#define ZETA_READ_ERR		1
	...
	double* gmin, double* gmax, double* ximin, double* ximax, double* zetaHat, int* zetaValid)
{
	...
	if (NULL != pzetaBinary)
	{
		...
		if ((e == ZETA_READ_OK) && (fread(rows,      sizeof(int),    1,     pzetaBinary) != 1))
		{
			e = ZETA_READ_ERR;
		}
		if ((e == ZETA_READ_OK) && (fread(cols,      sizeof(int),    1,     pzetaBinary) != 1))
		{
			e = ZETA_READ_ERR;
		}
		if ((e == ZETA_READ_OK) && (fread(gmin,      sizeof(double), 1,     pzetaBinary) != 1))
		{
			e = ZETA_READ_ERR;
		}
		if ((e == ZETA_READ_OK) && (fread(gmax,      sizeof(double), 1,     pzetaBinary) != 1))
		{
			e = ZETA_READ_ERR;
		}
		if ((e == ZETA_READ_OK) && (fread(ximin,     sizeof(double), 1,     pzetaBinary) != 1))
		{
			e = ZETA_READ_ERR;
		}
		if ((e == ZETA_READ_OK) && (fread(ximax,     sizeof(double), 1,     pzetaBinary) != 1))
		{
			e = ZETA_READ_ERR;
		}
		if (e == ZETA_READ_OK)
		...
		if ((e == ZETA_READ_OK) && (fread(zetaHat,   sizeof(double), nvals, pzetaBinary) != nvals))
		{
			e = ZETA_READ_ERR;
		}
		if ((e == ZETA_READ_OK) && (fread(zetaValid, sizeof(int),    nvals, pzetaBinary) != nvals))
		{
			e = ZETA_READ_ERR;
		}
		fclose(pzetaBinary);
