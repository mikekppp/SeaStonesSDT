...
static void save_xvtr (void) {
  for (int i = BANDS; i < BANDS + XVTRS; i++) {
    ...
    gtk_entry_set_text(GTK_ENTRY(lo_error[i]), f);
	  snprintf(f, sizeof(f), "%lld", xvtr->gaincalib);
    gtk_entry_set_text(GTK_ENTRY(gain[i]), f);

    if (protocol == ORIGINAL_PROTOCOL || protocol == NEW_PROTOCOL) {
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(disable_pa[i]), convertLongLongToInt(xvtr->disablePA));
    }
  }
...
void xvtr_menu(GtkWidget *parent) {
  ...
  gtk_grid_set_row_homogeneous(GTK_GRID(grid), G_FALSE);
  gtk_grid_set_column_homogeneous(GTK_GRID(grid), G_FALSE);
