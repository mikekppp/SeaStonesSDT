...
static void send_resp (int fd, char * msg) {
  ...
  while (length > 0) {
    ...
    //
    ssize_t rc = write(fd, msg, length);
...
static gboolean parse_extended_cmd (const char *command, CLIENT *client) {
  ...
  case 'F': //ZZFx
    ...
    case 'H': //ZZFH
      ...
      //ENDDEF
      if (command[4] == ';') {
		  snprintf(reply,  sizeof(reply), "ZZFH%05lld;", receiver[0]->filter_high);
        send_resp(client->fd, reply) ;
      } else if (command[9] == ';') {
    ...
    case 'L': //ZZFL
      ...
      //ENDDEF
      if (command[4] == ';') {
		  snprintf(reply,  sizeof(reply), "ZZFL%05lld;", receiver[0]->filter_low);
        send_resp(client->fd, reply) ;
      } else if (command[9] == ';') {
  ...
  case 'Z': //ZZZx
    ...
    case 'I': //ZZZI ANDROMEDA info
      ...
      //NOTE      or off (y=0).
      //ENDDEF
      implemented = G_FALSE;  // this command should never ARRIVE from the console
      break;
  ...
  case 'I':
    ...
    case 'F': { //IF
      ...
      if (can_transmit) {
		...
		snprintf(reply,  sizeof(reply), "IF%011lld%04lld%+06lld%d%d%d%02d%d%d%d%d%d%d%02d%d;",
  ...
  case 'S':
    ...
    case 'H': //SH
      ...
      if (command[2] == ';') {
        ...
        if (implemented) {
			snprintf(reply,  sizeof(reply), "SH%02lld;", fh);
          send_resp(client->fd, reply) ;
        }
      } else if (command[4] == ';') {
        // make sure filter is filterVar1
...
void disable_serial_rigctl (int id) {
  ...
  serial_client[id].running = G_FALSE;
