...
static void discovery(void) {
  ...
  GetPropLL0("radio_tcp_enable", tcp_enable);
#ifdef USBOZY
...
#include "saturnmain.h"
  ...
  if (devices == 0) {
    ...
    for (row = 0; row < devices; row++) {
      ...
      case STATE_SENDING:
        ...
        gtk_widget_set_sensitive(start_button, G_FALSE);
        break;
  ...
  long long num_hosts = 0;
  char str[128];
	GetPropLL0("num_hosts", num_hosts);
  ...
  *str = 0;
  ...
  gtk_entry_set_visibility(GTK_ENTRY(host_pwd), G_FALSE);
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(tcp_en), convertLongLongToInt(tcp_enable));
