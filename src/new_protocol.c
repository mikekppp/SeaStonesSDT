...
static void new_protocol_general(void) {
  ...
  {
    if ((rc = sendto(data_socket, general_buffer, sizeof(general_buffer), 0,
					 (struct sockaddr * )&base_addr, base_addr_length)) < 0)
	{
...
static void new_protocol_high_priority(void) {
  ...
  default:
    ...
    if (xmit && transmitter->puresignal && adc[2].adcAntenna == 6) { HPFfreq = 0LL; }
  ...
  if (!xmit && (device != NEW_DEVICE_ORION2 && device != NEW_DEVICE_SATURN) && adc[0].adcAntenna < 3) {
  ...
  //
  rxant = adc[0].adcAntenna;                      // 0,1,2  or 3,4,5

  if (xmit && transmitter->puresignal) {
    rxant = adc[2].adcAntenna;     // 0, 6, or 7
  }
    ...
    case HIGH_PRIORITY_TO_HOST_PORT:
      ...
      break;
		...
		case MIC_LINE_TO_HOST_PORT:
			int_bytesread = convertLongLongToInt(bytesread);
			saturn_post_micaudio(int_bytesread, mybuf);
			break;
