...
long long enable_protocol_1;
long long enable_protocol_2;
long long enable_soapy_protocol;
long long enable_stemlab;
long long enable_usbozy;
long long enable_saturn_xdma;
long long autostart;
...
void protocolsRestoreState(void) {
  ...
  // Set defauls
  //
  enable_protocol_1 = G_TRUE;
  enable_protocol_2 = G_TRUE;
  enable_stemlab = G_TRUE;
  enable_usbozy = G_TRUE;
  enable_soapy_protocol = G_TRUE;
  enable_saturn_xdma = G_TRUE;
  autostart = G_FALSE;
	GetPropLL0("enable_protocol_1",     enable_protocol_1);
	GetPropLL0("enable_protocol_2",     enable_protocol_2);
	GetPropLL0("enable_soapy_protocol", enable_soapy_protocol);
	GetPropLL0("enable_stemlab",        enable_stemlab);
	GetPropLL0("enable_usbozy",         enable_usbozy);
	GetPropLL0("enable_saturn_xdma",    enable_saturn_xdma);
	GetPropLL0("autostart",             autostart);
  clearProperties();
}
...
void configure_protocols(GtkWidget *parent) {
  ...
  gtk_header_bar_set_show_close_button(GTK_HEADER_BAR(headerbar), G_TRUE);
  gtk_header_bar_set_title(GTK_HEADER_BAR(headerbar), "piHPSDR - Protocols");
  g_signal_connect (dialog, "delete_event", G_CALLBACK (close_cb), NULL);
  g_signal_connect (dialog, "destroy", G_CALLBACK(close_cb), NULL);
  GtkWidget *content = gtk_dialog_get_content_area(GTK_DIALOG(dialog));
  GtkWidget *grid = gtk_grid_new();
  gtk_grid_set_row_homogeneous(GTK_GRID(grid), G_TRUE);
  row = 0;
  ...
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (b_enable_protocol_1), convertLongLongToInt(enable_protocol_1));
  ...
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (b_enable_protocol_2), convertLongLongToInt(enable_protocol_2));
  ...
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (b_autostart), convertLongLongToInt(autostart));
