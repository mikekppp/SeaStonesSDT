...
struct _BAND {
  ...
  long long gaincalib;                  // band dependent RX gain offset
  long long RxAntenna;                  // (ALEX) RX antenna
  long long TxAntenna;                  // (ALEX) TX antenna
  ...
  long long disablePA;                  // if 1, PA is disabled for this band
  //
  // Data that is automatically stored here, and applied whenever
  // a band change is done
  //
  long long preamp;                     // This is to support C25 attenuation storage
  long long dither;                     // This is to support C25 attenuation storage
  double gain;                    // position of the RF gain slider
  long long attenuation;                // position of the ATT slider
  long long alexAttenuation;            // if ALEX: attenuator (0/1/2/3 for 0/10/20/30 dB)
};
