...
static void choose_vfo_layout(void) {
  ...
  for (;;) {
    ...
    if ((vfl - vfo_layout_list) == vfo_layout_index) { rc = 0; }
  ...
  //
  if (vfo_layout_list[vfo_layout_index].width > VFO_WIDTH) {
    vfl = vfo_layout_list;
    ...
    for (;;) {
	  ...
	  vfo_layout_index = convertLongLongToInt(vfl - vfo_layout_list);
  ...
  //
  if (vfo_layout_list[vfo_layout_index].width < VFO_WIDTH - 50) {
    VFO_WIDTH -= 50;
    METER_WIDTH += 50;
  }
}
...
void radio_reconfigure_screen(void) {
  ...
  choose_vfo_layout();
  VFO_HEIGHT = vfo_layout_list[vfo_layout_index].height;
...
void radio_start_radio(void) {
  ...
  adc[1].overload = 0;
  adc[2].adcAntenna = 0;  // PS RX feedback antenna
...
void radio_apply_band_settings(int flag) {
  ...
  if (protocol == ORIGINAL_PROTOCOL || protocol == NEW_PROTOCOL) {
    adc[0].adcAntenna = band->RxAntenna;
...
static void radio_restore_state(void) {
  ...
  GetPropI0("vfo_layout",                                    vfo_layout_index);
  ...
  if (!radio_is_remote) {
    ...
    for (int i = 0; i < n_adc; i++) {
      GetPropLL1("radio.adc[%d].adcAntenna", i,              adc[i].adcAntenna);
      ...
      GetPropLL1("radio.adc[%d].alex_antenna", i,            adc[i].adcAntenna);
      GetPropI1("radio.adc[%d].filter_bypass", i,            adc[i].filter_bypass);
    }

    GetPropLL1("radio.adc[%d].alex_antenna", 2,              adc[2].adcAntenna);  // for PS RX feedback
    filterRestoreState();
    bandRestoreState();
    memRestoreState();
    vfo_restore_state();
...
void radio_save_state(void) {
  ...
  SetPropI0("vfo_layout",                                    vfo_layout_index);
  ...
  if (!radio_is_remote) {
    ...
    for (int i = 0; i < n_adc; i++) {
      SetPropI1("radio.adc[%d].adcAntenna", i,               adc[i].adcAntenna);
      ...
      SetPropI1("radio.adc[%d].alex_antenna", i,             adc[i].adcAntenna);
      SetPropI1("radio.adc[%d].filter_bypass", i,            adc[i].filter_bypass);
    }

    SetPropI1("radio.adc[%d].alex_antenna", 2,               adc[2].adcAntenna);  // for PS RX feedback
    filterSaveState();
    bandSaveState();
    memSaveState();
    vfo_save_state();
...
void my_combo_attach(GtkGrid *grid, GtkWidget *combo, int row, int col, int spanrow, int spancol) {
  if (optimize_for_touchscreen) {
    ...
    gtk_event_box_set_above_child(GTK_EVENT_BOX(eventbox), G_TRUE);
