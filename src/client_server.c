...
static int accumulated_round[2] = {G_FALSE, G_FALSE};
...
static void generate_pwd_hash(unsigned char *s, unsigned char *hash, const char *pwd) {
  ...
  //
  for  (unsigned long i = 0; i < pwdlen; i++) {
  ...
  for (int i = 0; i < 99999; i++) {
    ...
    for  (unsigned long j = 0; j < pwdlen; j++) {
...
static int connect_wait (int sockno, struct sockaddr * addr, socklen_t addrlen, struct timeval * timeout) {
  int res, opt;
...
static size_t recv_bytes(int s, char *buffer, size_t bytes) {
  size_t bytes_read = 0;
  int count = 0;

  while (bytes_read != bytes) {
    ssize_t rc = recv(s, &buffer[bytes_read], bytes - bytes_read, 0);
...
static int send_bytes(int s, char *buffer, int bytes) {
  ...
  while (bytes_sent != bytes) {
    ssize_t rc = send(s, &buffer[bytes_sent], bytes - bytes_sent, 0);
...
short remote_get_mic_sample(void) {
  //
...
void remote_send_txspectrum(void) {
  const float *samples;
  SPECTRUM_DATA spectrum_data;
  int numsamples = 0;
...
void send_band_data(int sock, int b) {
  ...
  data.gaincalib = convertLongLongToInt(band->gaincalib);
...
void send_bandstack_data(int sock, int b, int stack) {
  ...
  data.deviation = convertLongLongToInt(entry->deviation);
...
static void send_rx_data(int sock, int id) {
  ...
  data.filter_low            = convertLongLongToInt(rx->filter_low);
  data.filter_high           = convertLongLongToInt(rx->filter_high);
...
static void server_loop(void) {
  ...
  while (remoteclient.running) {
    ...
    // do a re-sync
    //
    size_t bytes_read = recv_bytes(remoteclient.socket, (char *)&header, sizeof(HEADER));
...
void send_filter_var(int s, int m, int f) {
  ...
  if (f == filterVar1 || f == filterVar2) {
    ...
    header.b1 = m;
    header.b2 = f;
    header.s1 = convertLongLongToInt(filters[m][f].low);
    header.s2 = convertLongLongToInt(filters[m][f].high);
    send_bytes(s, (char *)&header, sizeof(HEADER));
  }
}
...
void send_rx_filter_cut(int s, int id) {
  ...
  if (id < receivers) {
    header.s1  =  convertLongLongToInt(receiver[id]->filter_low);
    header.s2  =  convertLongLongToInt(receiver[id]->filter_high);
  }
...
void send_psatt(int s) {
  ...
  if (can_transmit) {
    ...
    header.s2 = to_short(adc[2].adcAntenna);
    send_bytes(s, (char *)&header, sizeof(HEADER));
  }
}
...
void send_rit(int s, int id) {
  ...
  header.s1 = convertLongLongToInt(vfo[id].rit);
  send_bytes(s, (char *)&header, sizeof(HEADER));
}
...
void send_xit(int s, int id) {
  ...
  header.s1 = convertLongLongToInt(vfo[id].xit);
  send_bytes(s, (char *)&header, sizeof(HEADER));
}
...
static void *listen_thread(void *arg) {
  ...
  while (server_running) {
    ...
    if (*s == 0x7F) {
      ...
      remoteclient.running = G_TRUE;

      for (int id = 0; id < RECEIVERS; id++) {
        remoteclient.send_rx_spectrum[id] = G_FALSE;
      }

      remoteclient.send_tx_spectrum = G_FALSE;
      //
...
static int check_vfo(void *arg) {
  ...
  if (!client_running) { return G_FALSE; }
...
static int remote_command(void *data) {
  ...
  case CMD_VFO_STEPSIZE: {
    const  U64_COMMAND *command = (U64_COMMAND *)data;
    int id = command->header.b1;
    long long step = from_ll(command->u64);
    vfo[id].step = step;
    g_idle_add(ext_vfo_update, NULL);
  }
  break;
  ...
  case CMD_PSATT: {
    if (can_transmit) {
      ...
      adc[2].adcAntenna = from_short(header->s2);
      schedule_high_priority();
    }
  }
  break;
