...
static int last_x;
static gboolean has_moved = G_FALSE;
static gboolean pressed = G_FALSE;
static gboolean making_active = G_FALSE;
  ...
  if (making_active) {
    ...
    if (pressed) {
      ...
      if (event->button == GDK_BUTTON_PRIMARY) {
        if (has_moved) {
          // drag
          vfo_move((long long)((float)(x - last_x)*rx->hz_per_pixel), G_TRUE);
        } else {
  ...
  if (!making_active && pressed && button_down) {
    ...
    if (moved) {
      if (has_moved || moved < -1 || moved > 1) {
        vfo_move((long long)((float)moved * rx->hz_per_pixel), G_FALSE);
        last_x = x;
        has_moved = G_TRUE;
      }
    }
