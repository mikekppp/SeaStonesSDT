...
void vfo_id_move(int id, long long hz, int round) {
  if (!locked) {
    ...
    if (vfo[id].ctun) {
	  ...
	  {
		int  	tmpStep = convertLongLongToInt(vfo[id].step);
        vfo[id].ctun_frequency = ROUND(vfo[id].ctun_frequency,
									   0,
									   tmpStep);
      }
	  ...
	  {
		  int  	tmpStep = convertLongLongToInt(vfo[id].step);
		  vfo[id].frequency = ROUND(vfo[id].frequency,
									0,
									tmpStep);
      }
...
void vfo_update(void) {
  ...
  //
  long long rxhigh = active_receiver->filter_high;
  long long rxlow = active_receiver->filter_low;
  long long w = rxhigh - rxlow;
  ...
  if (w < 0) {
    ...
    w = 10 * ((w + 5) / 10); // between 0 and 990
	  snprintf(wid, sizeof(wid), "%3lld", w);
  } else if (w < 9950) {
    w = 100 * ((w + 50) / 100); // between 1000 and 9900
    // print "2.7k" for a width of 2700
	  snprintf(wid, sizeof(wid), "%lld.%lldk", w / 1000, (w % 1000) / 100);
  } else if (w < 99999) {
    // print "11k" for a width of 11000
    w = (w + 500) / 1000;
	  snprintf(wid, sizeof(wid), "%2lldk", w);
  } else {
    snprintf(wid, sizeof(wid), "+++");
  }
  ...
  const VFO_BAR_LAYOUT *vfl = &vfo_layout_list[vfo_layout_index];
  //
  ...
  if (m != modeFMN && vfl->filter_x != 0) {
    double range;
    double s, x1, x2;
    long long def_low = band_filter->low;
    long long def_high = band_filter->high;
...
#endif
  int oob = 0;
  long long f_m; // MHz part
  long long f_k; // kHz part
  long long f_h; // Hz  part
  ...
  if (vfl->vfo_a_x != 0) {
    ...
    if (txvfo == 0 && oob) {
      ...
      cairo_restore(cr);
		snprintf(temp_text, sizeof(temp_text), "%0lld.%03lld", f_m, f_k);
      cairo_show_text(cr, temp_text);
      cairo_set_font_size(cr, vfl->size2);
		snprintf(temp_text, sizeof(temp_text), "%03lld", f_h);
      cairo_show_text(cr, temp_text);
    }
  }
...
static gboolean vfo_press_event_cb (GtkWidget *widget, GdkEventButton *event, gpointer data) {
  ...
  case GDK_BUTTON_PRIMARY:
    ...
    if (event->x >= abs(vfo_layout_list[vfo_layout_index].vfo_b_x)) { v = VFO_B; }
