...
int zoompan_active_receiver_changed(void *data) {
  if (display_zoompan) {
    ...
    if (active_receiver->zoom == 1) {
      gtk_widget_set_sensitive(pan_scale, G_FALSE);
    }
...
static void zoom_value_changed_cb(GtkWidget *widget, gpointer data) {
  ...
  if (active_receiver->zoom == 1) {
    ...
    gtk_widget_set_sensitive(pan_scale, G_TRUE);
  }
...
GtkWidget *zoompan_init(int my_width, int my_height) {
  ...
  gtk_grid_set_row_homogeneous(GTK_GRID(zoompan), G_FALSE);
  gtk_grid_set_column_homogeneous(GTK_GRID(zoompan), G_TRUE);
  ...
  gtk_scale_set_draw_value (GTK_SCALE(pan_scale), G_FALSE);
