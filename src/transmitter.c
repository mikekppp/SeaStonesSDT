...
static gboolean tx_update_display(gpointer data) {
  ...
  if (tx->displaying) {
    ...
    g_mutex_unlock(&tx->display_mutex);
    return G_TRUE; // keep going
  }

  return G_FALSE; // no more timer events
}
...
void tx_create_dialog(TRANSMITTER *tx) {
  ...
  gtk_header_bar_set_show_close_button(GTK_HEADER_BAR(headerbar), G_FALSE);
