...
void show_popup_slider(enum ACTION action, int rx, double min, double max, double delta, double value,
  ...
  if (scale_status == NO_ACTION) {
    ...
    gtk_widget_set_sensitive(popup_scale, G_FALSE);
...
void sliders_filter_width(int id, long long width) {
  if (id > receivers) { return; }
...
void sliders_filter_shift(int id, long long shift) {
  if (id > receivers) { return; }
...
void sliders_filter_high(int id, long long var) {
  if (id > receivers) { return; }
...
void sliders_filter_low(int id, long long  var) {
  if (id > receivers) { return; }
...
void sliders_diversity_gain(void) {
  //
  // This ONLY moves the slider and updates the checkbutton
  //
...
void sliders_diversity_phase(void) {
  //
  // This ONLY moves the slider and updates the checkbutton
  //
...
GtkWidget *sliders_init(int my_width, int my_height) {
  ...
  gtk_grid_set_row_homogeneous(GTK_GRID(sliders), G_FALSE);
  gtk_grid_set_column_homogeneous(GTK_GRID(sliders), G_TRUE);
  ...
  gtk_grid_set_column_homogeneous(GTK_GRID(c25_grid), G_TRUE);
  //
