...
static void show_hf(void) {
  ...
  gtk_grid_set_column_homogeneous(GTK_GRID(mygrid), G_FALSE);
  gtk_grid_set_row_homogeneous(GTK_GRID(mygrid), G_TRUE);
  ...
  for (int i = 0; i <= bands + 2; i++) {
    ...
    if (strlen(band->title) > 0) {
      ...
      gtk_combo_box_set_active(GTK_COMBO_BOX(rxcombo), convertLongLongToInt(band->RxAntenna));
      ...
      gtk_combo_box_set_active(GTK_COMBO_BOX(txcombo), convertLongLongToInt(band->TxAntenna));
...
void ant_menu(GtkWidget *parent) {
  ...
  if (device != SOAPYSDR_USB_DEVICE) {
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(hf_rb), G_TRUE);
  ...
  if (device == SOAPYSDR_USB_DEVICE) {
    ...
    for (int id = 0; id < RECEIVERS; id++) {
      if (radio->soapy.rx[id].antennas > 0) {
        ...
        gtk_combo_box_set_active(GTK_COMBO_BOX(box), adc[id].adcAntenna);
