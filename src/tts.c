...
void tts_freq(void) {
  ...
  hertz = freq - 1000 * kilo;
	snprintf(msg, sizeof(msg), "Frequency %ld.%03lld kilo hertz", kilo, hertz);
  tts_send(msg);
}
...
void tts_filter(void) {
  char msg[128];
  long long w = active_receiver->filter_high - active_receiver->filter_low;
	snprintf(msg, sizeof(msg), "Filter width %lld Hertz", w);
  tts_send(msg);
}
